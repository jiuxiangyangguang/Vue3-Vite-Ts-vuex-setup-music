...
MyServer.prototype.parseRouter = function (name: string, urlObj: any) {
  ...
  Object.keys(urlObj).forEach(apiName => {
    this[name][apiName] = this.sendMes.bind(this, name, apiName, urlObj[apiName].url, urlObj[apiName].type)
  })
}

MyServer.prototype.sendMes = function (moduleName: string, apiName: string, url: string, type: string, config: any) {
